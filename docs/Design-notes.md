...
main.rs now captures real (x, y) coordinates on mouse_click events (tracked
from the last MouseMove position). Raw events in nexi_events.db correctly
carry exact pixel coordinates.
...
not an oversight - but it needs a real decision before pattern mining
(stage 4) gets built on top of it.
...
last time. If Token::Click carries exact (x, y), two functionally identical
clicks will almost never match as "the same pattern" - pattern mining
(stage 4) would find nothing, since it works by matching identical token
sequences.
